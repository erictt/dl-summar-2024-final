...
def main():
    ...
    def post_processing_function(examples, features, predictions, stage="eval"):
        ...
        references = [
            {"id": str(ex["id"]), "answers": ex[answer_column_name]} for ex in examples
        ]
    ...
    if (
        ...
        and training_args.metric_for_best_model not in accepted_best_metrics
    ):
        warnings.warn(
            f"--metric_for_best_model should be set to one of {accepted_best_metrics}"
        )
